...
void default_load(appliedforce* F, int nData) /** default stuff for load **/
{
    ...
    F -> Fx = calloc(sizeof(double),F->n);
    F -> Fy = calloc(sizeof(double),F->n);
    F -> Mz = calloc(sizeof(double),F->n);
                            ...
                            int nData, double *px, double *py, double *tx, double *ty, double*FxTip, double*FyTip, double*MzTip)/**load vector; number of load data, load values, assigned time interval values**/
{
    ...
    /**for validation purposes, sample is Soesianawati et al 1986**/
    /**Kx and Ky are at yielding, Kz is at cracking*/
    double Kx=2.58e7 , Ky=1.19e9 , Kz=2.87e7 ;
Ky = 2.5934E+09, Kx = 3.3828E+07, Kz = 7.5772E+06;
...
//		k1 = df(ti,nComp, yi, nData,px,py,tx,ty,FxTip, FyTip, MzTip);
		...
		ynew = &m[i+1][1];
        //velocities, updated at halfstep
        k1 = df(ti+0.5*h,nComp, yi, nData,px,py,tx,ty,FxTip, FyTip, MzTip);
        for(j=nEqn/2.0;j<nEqn;j++){
            yk[j] = yi[j] + k1[j]*0.5*h;
        }   //option to store velocity values here (midsteps)

        //positions, updated at fullstep
        k2 = df(ti+h,nComp, yk, nData,px,py,tx,ty,FxTip, FyTip, MzTip);
...
/**
leapfrog integration (step-wise) - start
 **/
...
appliedforce* forceinfo) /**load vector; number of load data, load values, assigned time interval values**/
{
    ...
    int nData = forceinfo->n;
    double *px = forceinfo->Fx;
    double *py = forceinfo->Fy;

    double*FxTip, *FyTip, *MzTip;
    ...
    int i, j;                                                   /**counters**/
	double h = 0.1;                         /**interval width - arbitrary (temp) **/
//	double **m = NewMatrix(1, nEqn+1);                    /**answer matrix; t, y1, y2, ..., yn, y1dot, y2dot, ..., yndot**/
                                                                /**size+1 accounts for column for t, as well as row t=0**/
	double ti, *yk = calloc(nEqn, sizeof(double));              /**ti - first column, yk - RK evaluations of y**/
	double *k1, *k2, *k3, *k4;                                  /**RK4 parameters**/
    double *ynew, *yi;	                                        /**pointers to answer matrix**/
    ynew = calloc(nEqn, sizeof(double));
//	m[0][0] = 0;                                                /**answer matrix, first vector takes points a to b**/
//	memcpy(&m[0][1], ya, nEqn*sizeof(double));                  /**copy initial conditions to answer matrix**/

	double *vh = calloc(nEqn,sizeof(double));                   /**halfstep velocity**/
...
//		k1 = df(ti,nComp, yi, nData,px,py,tx,ty,FxTip, FyTip, MzTip);
        ...
        //velocities, updated at halfstep
        k1 = df(a+0.5*h,nComp, yi, nData,px,py,a+0.5*h,a+0.5*h
//                ,FxTip, FyTip, MzTip
                );
        for(j=nEqn/2.0;j<nEqn;j++){
            yk[j] = yi[j] + k1[j]*0.5*h;
        ...
        //positions, updated at fullstep
        k2 = df(a+h,nComp, yk, nData,px,py,a+h,a+h
//                ,FxTip, FyTip, MzTip
                );
        for(j=0;j<nEqn/2.0;j++){
            ynew[j] = yi[j] + k2[j]*h;
...
/**
leapfrog integration (step-wise) - step
 **/
double** LeapFrogStep(
double * (*df)(),   /**function derivative**/
int nComp,                                                         /**number of components**/
...
double* df_SmallAngleApproxNew(double t,
                            int nComp,  /**number of components**/
                            double *y,  /**y is of size "order" (6 x no. of elems)**/
                            int nData, double *px, double *py, double *tx, double *ty,
                            rigidbody *RigidBody
)/**load vector; number of load data, load values, assigned time interval values**/
{
    int order, nEqn; order = 3*nComp;nEqn = 2*order;
    double *theta;
    double *dy=NewVector(nEqn);               /**answer vector**/
    ...
    /**for validation purposes, sample is Soesianawati et al 1986**/
    /**Kx and Ky are at yielding, Kz is at cracking*/
    double Kx = 3.3828E+07,Ky = 2.5934E+09,Kz = 7.5772E+06;
